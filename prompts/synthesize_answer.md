...
How to write `evidence_summary`:
- Open with one sentence that directly answers the question (e.g. "Yes — …", "Modestly — …", "The evidence is mixed: …"). If the evidence can't answer it, say that in this first sentence.
- Then give the key supporting findings, preferring concrete results (effect sizes, percentages, trial or cohort names) over background. Note important caveats or limitations the evidence mentions.
- Leave out anything that doesn't help answer this specific question — no general disease background, regulatory or funding asides, unless the question asks for them.
- 4-7 sentences. Briefly explain each technical term or abbreviation the first time you use it.
...
2. Every factual claim in `evidence_summary` must be grounded in at least one of the provided chunks. List EVERY chunk_id you took any fact, number, or finding from in the `citations` field (that field only — never in the prose). Do not cite a chunk_id that isn't listed above.
...
Reminder — the question you must answer directly is:
{{question}}
